...
自動追蹤澳幣兌台幣匯率，到價提醒推送到 Telegram。

## 功能

- ⏰ 每 4 小時自動抓取 AUD/TWD 匯率（免費 API，無需 API Key）
- 📊 歷史數據自動累積（最多保留一年）
- 🔔 到價 Telegram 提醒（跌破 / 突破指定價位）
- 📉 跌破 30 日均線自動通知
- 📏 自動計算 30/90 日均線、歷史百分位

## 設定步驟

### 1. Fork 或建立此 Repo

把這些檔案推上你的 GitHub repo。
...
到 repo → Actions → 點擊 "I understand my workflows, go ahead and enable them"

### 4. 手動測試

Actions → "Fetch AUD/TWD Rate" → Run workflow → 看有沒有成功

如果 Telegram 收到通知就代表設定成功！
...
```json
{
  "below": [21.0, 21.5, 22.0],    // 跌破這些價位時通知（換匯好時機）
  "above": [23.0, 23.5, 24.0],    // 突破這些價位時通知（澳幣變貴）
  "ma_cross": true,                // 跌破 30 日均線時通知
  "last_alerted": {}               // 系統自動管理，不要手動改
}
```

## 資料來源

- 主要：[Frankfurter API](https://frankfurter.dev)（歐洲央行數據）
- 備用：[ExchangeRate-API](https://open.er-api.com)

兩者都免費、無需 API Key。

## 之後的擴充計畫

- Phase 2：手機版 Dashboard（走勢圖、均線、RBA 日曆）
- Phase 3：匯差計算機（玉山/Revolut/Wise 多管道比價）
